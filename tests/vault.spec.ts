...
describe("Vault Contract", () => {
  ...
  it("should handle deposits correctly", () => {
    const depositAmount = 1_000_000u64;
    ...
    const shares = new Args(result).nextU64().unwrap();
    expect(shares).toBe(depositAmount);
    
    const totalAssetsResult = totalAssets(new StaticArray<u8>(0));
    const totalAssetsValue = new Args(totalAssetsResult).nextU128().unwrap();
    ...
    const userShares = new Args(userSharesResult).nextU128().unwrap();
  ...
  it("should calculate share price correctly", () => {
    ...
    const deposit1 = new Args().addU64(1_000_000u64);
    deposit(deposit1.serialize());
    
    const syncArgs = new Args().addU128(u128.from(1_200_000u64));
    ...
    const deposit2 = new Args().addU64(1_000_000u64);
    const result = deposit(deposit2.serialize());
    
    const shares = new Args(result).nextU64().unwrap();
    expect(shares).toBeLessThan(1_000_000u64);
    expect(shares).toBeGreaterThan(800_000u64);
  });
  
  it("should prevent unauthorized sync", () => {
    Context.setCaller("AS1UnauthorizedUser");
    
    const syncArgs = new Args().addU128(u128.from(1_000_000u64));
  ...
  it("should handle zero deposits", () => {
    ...
    const depositArgs = new Args().addU64(0u64);
  ...
  it("should handle insufficient balance", () => {
    Context.setCaller("AS1TestUser");
    Context.setBalance(100u64);
    
    const depositArgs = new Args().addU64(1_000_000u64);
